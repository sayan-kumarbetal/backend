...
const generateAccessAndRefreshTokens = async (useId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Something went generating access and refresh tokens"
    );
  }
};
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  //send cookie
  //sueponse sen sucessfully login
...
const getCurrentUser = asyncHandler(async (req, res) => {
  return res
    .status(200)
    .json(200, req.user, "Current user fetchde sucessfully");
});
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, channel[0], "User channell fetched sucessfully")
    );
});
